...
import com.mystic.rockyminerals.api.set.stone.VanillaStoneTypes;
...
import static com.mystic.rockyminerals.misc.HardcodedBlockType.isKnownVanillaMineral;
import static com.mystic.rockyminerals.misc.HardcodedBlockType.isKnownVanillaStone;
...
public class ResourcesGenerator {
    ...
    public static void generateResources(ResourceManager manager, ResourceSink sink) {

        /// Creating textures for all RockTypes
        for (StoneType currentType : StoneTypeRegistry.INSTANCE) {
            if (isKnownVanillaStone(currentType)) continue; // Skip the vanilla StoneType like minecraft:stone
            generateTexture("saltstone", currentType, Saltstone_Templates, sink, manager);
        }
        for (MineralType currentType : MineralTypeRegistry.INSTANCE) {
            if (isKnownVanillaMineral(currentType)) continue; // Skip the vanilla MineralType like minecraft:amethyst
        ...
        for (BlockType currentType : rockTypes) {
            if (currentType instanceof StoneType stoneType && isKnownVanillaStone(stoneType)) continue;
            BlockType baseType = VanillaStoneTypes.SALTSTONE;
            ...
            if (!decorativeItemTypes.isEmpty()) {
                generateStandardItemModels(currentType, decorativeItemTypes.get(currentType), baseType,
                        makeModelTransformer(baseType, manager), manager, sink
                );
            }
        }
    }
        ...
        try (TextureImage stoneImage = TextureImage.open(manager,
            ...
            for (TextureInfo textureInfo : textureTemplates) {
                ...
                try (TextureImage baseTexture = TextureImage.open(manager, textureInfo.textureResLoc())) {
                    ...
                    sink.addTextureIfNotPresent(manager, newResLoc, finishedTexture);
                }
                catch (IOException e) {
    ...
    public static Supplier<TextureImage> maybePostProcessStoneTexture(StoneType stoneType, ResourceManager manager, Supplier<TextureImage> textureSupplier) {
        return () -> {
            TextureImage texture = textureSupplier.get();
            return mossyTexture(texture, manager, stoneType);
        };
    }

    private static TextureImage mossyTexture(TextureImage textureImage, ResourceManager manager, StoneType stoneType) {
